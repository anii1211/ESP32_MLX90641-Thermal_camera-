...
SoftWire softWire = SoftWire(41,42);

uint8_t i2cTxBuf[128];
uint8_t i2cRxBuf[128];

void MLX9064x_I2CInit()
{
    //softWire.enablePullups();
    //softWire.setDelay_us(5);
    softWire.begin();
    softWire.setRxBuffer(i2cRxBuf,128);
    softWire.setTxBuffer(i2cTxBuf,128);
}
...
int MLX9064x_I2CRead(uint8_t _deviceAddress, unsigned int startAddress, unsigned int nWordsRead, uint16_t *data)
{
    ...
    {
        softWire.beginTransmission(_deviceAddress);
        softWire.write(startAddress >> 8);   // MSB
        softWire.write(startAddress & 0xFF); // LSB
        if (softWire.endTransmission(false) != 0)
        { // Do not release bus
        ...
        softWire.requestFrom((uint8_t)_deviceAddress, numberOfBytesToRead);
        if (softWire.available())
        {
            for (uint16_t x = 0; x < numberOfBytesToRead / 2; x++)
            {
                // Store data into array
                data[dataSpot] = softWire.read() << 8; // MSB
                data[dataSpot] |= softWire.read();     // LSB
...
void MLX9064x_I2CFreqSet(int freq)
{
    ...
    softWire.setClock((long)1000 * freq);
}
...
{
    softWire.beginTransmission((uint8_t)_deviceAddress);
    softWire.write(writeAddress >> 8);   // MSB
    softWire.write(writeAddress & 0xFF); // LSB
    softWire.write(data >> 8);           // MSB
    softWire.write(data & 0xFF);         // LSB
